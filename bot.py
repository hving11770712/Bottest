...
# === CẤU HÌNH ===
TOKEN = "7593769015:AAHrVgmtYOXhVLWcPh9g0wv6Uy_IA0OYS-Q"
...
async def id_cmd(message: types.Message):
    ...
    text = [
        ...
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
        f"👤 Tên: {name}",
        f"🔢 ID: <code>{message.text.lower()}</code> {uid}",
        f"📊 Trạng Thái: {status}",
        f"⏰ Hạn Dùng: {exp}",
...
async def add_user(message: types.Message):
    ...
    save_activated_users()
    await message.reply(f"✅ Đã cấp quyền cho ID {user_id} ({'vĩnh viễn' if days == 'infinity' else f'{days} ngày'})\n\n⏰ Thời gian cấp: {now}")
